...
public class Exercise2 {
    ...
    public void personHasNotEmptyLastNameAndFirstName() {
        ...
        Predicate<Person> validate =
                person -> !"".equals(person.getFirstName()) && !"".equals(person.getLastName());
    ...
    private <T> Predicate<T> emptyStringPredicate(Function<T, String> stringExtractor) {
        return (person) -> stringExtractor.apply(person).isEmpty();
    }

    @Test
    public void personHasNotEmptyLastNameAndFirstNameUsingLogicalOperators() {
        Predicate<Person> personHasEmptyFirstName = emptyStringPredicate(Person::getFirstName);
        Predicate<Person> personHasEmptyLastName = emptyStringPredicate(Person::getLastName);
